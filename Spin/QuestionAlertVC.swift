...
class QuestionAlertVC: UIViewController {
    ...
    @objc fileprivate func loadNibAlertController() -> [AnyObject]?{
        ...
        if let bundleURL = podBundle.url(forResource: "QuestionAlertVC", withExtension: "bundle"){
            
            if let bundle = Bundle(url: bundleURL) {
                return bundle.loadNibNamed("QuestionAlertVC", owner: self, options: nil) as [AnyObject]?
            }
            else {
        ...
        else if let nib = podBundle.loadNibNamed("QuestionAlertVC", owner: self, options: nil) as [AnyObject]?{
            return nib
        }
        else{
